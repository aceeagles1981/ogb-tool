...
async function handleEmailFile(files){
  ...
  try{
    ...
    // Pre-fill create new insured form — use AI-extracted insuredName first
    const newNameEl = document.getElementById('single-new-name');
    if(newNameEl){
      const aiName = data.insuredName || (data.note && data.note.insuredName) || '';
      const matchName = data.match && data.match.matched_name || '';
      const subj = (data.subject||'').replace(/^(RE:|FW:|re:|fw:|AW:|SV:)\s*/i,'').split(' - ')[0].trim();
      newNameEl.value = aiName || matchName || subj;
    }
    ...
    fillIngestRiskDraft({
      assured_name: ((data.match && data.match.matched_name) || (data.note && data.note.insuredName) || ''),
      status: inferRiskStatusFromText(note.statusChange||''),
      accounting_year: parseInt((note.date||'').slice(0,4),10) || new Date().getFullYear(),
      notes: note.summary||''
    });
    setTimeout(function(){ generateIngestRiskDraft(); generateIngestTaskDrafts(); }, 50);
    const confColour = {high:'var(--ok)',medium:'var(--warn)',low:'var(--err)'}[match.confidence]||'var(--text2)';
    document.getElementById('en-match-badge').innerHTML = match.matched_name
      ? `<div style="padding:8px 12px;border-radius:6px;background:var(--bg);border:1px solid var(--border);font-size:12px"><span style="color:${confColour};font-weight:600">${match.confidence==='high'?'✓':match.confidence==='medium'?'~':'?'} ${match.confidence}</span> — matched to <strong>${match.matched_name}</strong><br><span class="muted">${match.reason||''}</span></div>`
      : '<div class="notice warn" style="margin:0">No automatic match — please select account below.</div>';
    // Auto-compliance on high-confidence match
    if(match.confidence==='high' && match.matched_id) {
    ...
    const sel = document.getElementById('en-insured-select');
    sel.innerHTML='<option value="">— select insured —</option>';
    [...ent.insureds].sort((a,b)=>a.name.localeCompare(b.name)).forEach(i=>{
      const o=document.createElement('option');
      o.value=i.id; o.textContent=i.name;
      if(i.id===match.matched_id) o.selected=true;
      sel.appendChild(o);
    });
    populateEnquirySelect(match.matched_id);
    sel.onchange=()=>populateEnquirySelect(sel.value);
...
function populateEnquirySelect(insId){
  const ent=entGetState();
  const ins=ent.insureds.find(i=>i.id===insId);
...
function saveEmailNote(){
  const insId=document.getElementById('en-insured-select').value;
  const enqId=document.getElementById('en-enquiry-select').value;
  if(!insId){showNotice('Please select an insured','err');return;}
  const ent=entGetState();
  const ins=ent.insureds.find(i=>i.id===insId);
  if(!ins){showNotice('Insured not found','err');return;}
  const actionsRaw=document.getElementById('en-actions').value.trim();
  const note={
    id:'note-'+Date.now(),
    date:document.getElementById('en-date').value.trim(),
    handler:document.getElementById('en-handler').value.trim(),
    parties:document.getElementById('en-parties').value.trim(),
    summary:document.getElementById('en-summary').value.trim(),
    actions:actionsRaw?actionsRaw.split('\n').map(s=>s.trim()).filter(Boolean):[],
    statusChange:document.getElementById('en-status').value.trim(),
    enquiryId:enqId||'',
    docType:(window._lastNoteData&&window._lastNoteData.docType)||'general-correspondence',
    terms:(window._lastNoteData&&window._lastNoteData.terms)||{}
  };
  if(!ins.notes) ins.notes=[];
  if(isDuplicateNote(ins, note)){
    showNotice('Duplicate note detected — already saved for this date/parties','err');
    return;
  }
  ins.notes.push(note);
  // Save document if present
  if(window._lastNoteData && window._lastNoteData.document && window._lastNoteData.document.content){
    if(!ins.documents) ins.documents=[];
    ins.documents.push({
      id:'doc-'+Date.now(),
      date:note.date||'',
      type:window._lastNoteData.document.type||'',
      title:window._lastNoteData.document.title||'',
      content:window._lastNoteData.document.content||'',
      source:'single-ingest'
    });
  }
  // Save loss record if present
  if(window._lastNoteData && window._lastNoteData.lossRecord && window._lastNoteData.lossRecord.length){
    if(!ins.lossRecord) ins.lossRecord=[];
    window._lastNoteData.lossRecord.forEach(lr=>{
      if(!ins.lossRecord.some(x=>x.year===lr.year)) ins.lossRecord.push(lr);
    });
  }
  if(note.statusChange&&enqId){
    const enq=ins.enquiries.find(e=>e.id===enqId);
    if(enq){
      const parts=note.statusChange.split(/→|to|->/i);
      if(parts.length===2) enq.status=parts[1].trim();
    }
  }
  autoTickPostBind(ent, ins, note);
  entSave(ent);
  // Save contacts returned from backend (stored during handleEmailFile)
  if(window._lastEmailContacts && window._lastEmailContacts.length){
    upsertContacts(window._lastEmailContacts, ins.name);
    window._lastEmailContacts = null;
  }
  document.getElementById('email-result-card').style.display='none';
  const saved=document.getElementById('email-saved-notice');
  saved.textContent='✓ Note saved to '+ins.name+(note.statusChange?' · Status: '+note.statusChange:'');
  saved.style.display='block';
  document.getElementById('dz-email').classList.remove('loaded');
  document.getElementById('dz-email-label').textContent='Drop .msg file here';
  document.getElementById('fi-email').value='';
...
function getIngestActionLines(){
  var raw = (document.getElementById('en-actions')||{}).value || '';
  return raw.split(/\n/).map(function(s){ return s.trim(); }).filter(Boolean);
}

function renderIngestTaskDrafts(tasks){
  tasks = Array.isArray(tasks) ? tasks : [];
  window._lastTaskDrafts = tasks;
  var wrap = document.getElementById('en-task-list');
  var msg = document.getElementById('en-task-msg');
  if(!wrap) return;
  if(!tasks.length){
    wrap.innerHTML = '<div class="muted" style="font-size:11px">No task drafts yet.</div>';
    if(msg) msg.textContent = 'No suggested actions were available to turn into tasks.';
    return;
  }
  wrap.innerHTML = tasks.map(function(t, idx){
    return '<div style="border:1px solid var(--border);border-radius:8px;padding:10px;background:var(--bg)">' +
      '<div style="display:grid;grid-template-columns:1.8fr 1fr 1fr 1fr;gap:8px;align-items:end">' +
      '<div><div class="muted" style="font-size:10px;margin-bottom:3px">TITLE</div><input type="text" data-task-field="title" data-task-idx="'+idx+'" value="'+escapeHtml(t.title||'')+'" style="font-size:12px;padding:5px 8px"></div>' +
      '<div><div class="muted" style="font-size:10px;margin-bottom:3px">OWNER</div><input type="text" data-task-field="owner" data-task-idx="'+idx+'" value="'+escapeHtml(t.owner||'')+'" style="font-size:12px;padding:5px 8px"></div>' +
      '<div><div class="muted" style="font-size:10px;margin-bottom:3px">PRIORITY</div><select data-task-field="priority" data-task-idx="'+idx+'" style="font-size:12px;padding:5px 8px">' +
        '<option value="low"'+((t.priority==='low')?' selected':'')+'>Low</option>' +
        '<option value="normal"'+((!t.priority || t.priority==='normal')?' selected':'')+'>Normal</option>' +
        '<option value="high"'+((t.priority==='high')?' selected':'')+'>High</option>' +
        '<option value="urgent"'+((t.priority==='urgent')?' selected':'')+'>Urgent</option>' +
      '</select></div>' +
      '<div><div class="muted" style="font-size:10px;margin-bottom:3px">DUE DATE</div><input type="date" data-task-field="due_date" data-task-idx="'+idx+'" value="'+escapeHtml(t.due_date||'')+'" style="font-size:12px;padding:5px 8px"></div>' +
      '</div>' +
      '<div style="margin-top:8px"><div class="muted" style="font-size:10px;margin-bottom:3px">DESCRIPTION</div><textarea data-task-field="description" data-task-idx="'+idx+'" rows="2" style="font-size:12px;padding:5px 8px;resize:vertical">'+escapeHtml(t.description||'')+'</textarea></div>' +
      '</div>';
  }).join('');
  wrap.querySelectorAll('[data-task-field]').forEach(function(el){
    el.addEventListener('input', syncIngestTaskDraftInputs);
    el.addEventListener('change', syncIngestTaskDraftInputs);
  });
  if(msg) msg.textContent = tasks.length + ' task draft' + (tasks.length===1?'':'s') + ' ready to save with the risk.';
}

function syncIngestTaskDraftInputs(){
  var wrap = document.getElementById('en-task-list');
  if(!wrap || !Array.isArray(window._lastTaskDrafts)) return;
  wrap.querySelectorAll('[data-task-field]').forEach(function(el){
    var idx = parseInt(el.getAttribute('data-task-idx')||'-1',10);
    var field = el.getAttribute('data-task-field');
    if(idx >= 0 && window._lastTaskDrafts[idx]){
      window._lastTaskDrafts[idx][field] = el.value;
    }
  });
}

async function generateIngestTaskDrafts(){
  var btn = document.getElementById('en-task-generate-btn');
  var lines = getIngestActionLines();
  var handler = (document.getElementById('en-handler')||{}).value || '';
  try{
    if(btn){ btn.disabled = true; btn.textContent = 'Generating...'; }
    if(!lines.length){
      renderIngestTaskDrafts([]);
      return;
    }
    var tasks = lines.map(function(line){
      var lower = line.toLowerCase();
      var priority = /urgent|asap|today|immediately/.test(lower) ? 'urgent' :
                     /chase|firm|quoted|bind|renewal|deadline|before/.test(lower) ? 'high' : 'normal';
      return {
        ...
        due_date: '',
        status: 'open',
        source: 'ingest_ai_actions'
      };
    });
    renderIngestTaskDrafts(tasks);
  } catch(e){
    console.error(e);
    var msg = document.getElementById('en-task-msg');
...
async function saveIngestTaskDrafts(riskId){
  syncIngestTaskDraftInputs();
  var tasks = (window._lastTaskDrafts || []).filter(function(t){ return (t.title||'').trim(); });
  ...
  for(var i=0;i<tasks.length;i++){
    ...
    await apiFetch('/tasks', {
      ...
      body: JSON.stringify({
        ...
        source: t.source || 'ingest_ai_actions'
      })
    });
    saved += 1;
  }
...
function fillIngestRiskDraft(fields){
  fields = fields || {};
  var assured = fields.assured_name || fields.display_name || '';
  if(!assured){ assured = (document.getElementById('single-new-name')||{}).value || ''; }
  if(!assured){
    var sel = document.getElementById('en-insured-select');
    if(sel && sel.value && sel.selectedOptions && sel.selectedOptions[0]) assured = sel.selectedOptions[0].textContent || '';
  }
  document.getElementById('en-risk-assured').value = assured || '';
  document.getElementById('en-risk-product').value = fields.product || '';
  document.getElementById('en-risk-status').value = fields.status || inferRiskStatusFromText(document.getElementById('en-status').value || '');
  document.getElementById('en-risk-year').value = fields.accounting_year || new Date().getFullYear();
  document.getElementById('en-risk-ccy').value = fields.currency || 'USD';
  document.getElementById('en-risk-inception').value = fields.inception_date || '';
  document.getElementById('en-risk-expiry').value = fields.expiry_date || '';
  document.getElementById('en-risk-premium').value = fields.gross_premium || '';
  document.getElementById('en-risk-brokerage').value = fields.brokerage_pct || '';
  document.getElementById('en-risk-retained').value = (fields.retained_pct != null ? fields.retained_pct : 100);
  document.getElementById('en-risk-comm').value = fields.estimated_gbp_commission || '';
  document.getElementById('en-risk-notes').value = fields.notes || document.getElementById('en-summary').value || '';
  window._ingestLinkedRiskId = null;
  renderIngestRiskMatches([], null);
}

function getCurrentIngestRiskPayload(){
  var assured = (document.getElementById('en-risk-assured').value || '').trim();
  var year = parseInt(document.getElementById('en-risk-year').value || new Date().getFullYear(), 10);
  return {
    ...
    handler: (document.getElementById('en-handler').value || '').trim(),
    region: '',
    product: (document.getElementById('en-risk-product').value || '').trim(),
    layer: '',
    status: document.getElementById('en-risk-status').value || 'submission',
    accounting_year: year,
    inception_date: document.getElementById('en-risk-inception').value || null,
    expiry_date: document.getElementById('en-risk-expiry').value || null,
    currency: document.getElementById('en-risk-ccy').value || 'USD',
    gross_premium: parseFloat(document.getElementById('en-risk-premium').value) || null,
    brokerage_pct: parseFloat(document.getElementById('en-risk-brokerage').value) || null,
    retained_pct: parseFloat(document.getElementById('en-risk-retained').value) || null,
    estimated_gbp_commission: parseFloat(document.getElementById('en-risk-comm').value) || null,
    notes: (document.getElementById('en-risk-notes').value || '').trim(),
    ai_extracted: window._lastRiskDraft || {},
    needs_review: true,
    review_reason: ((window._lastRiskDraft||{}).review_reason || 'Created from email ingest draft')
  };
}

function riskStatusRank(s){
  var map = {submission:1,in_market:2,quoted:3,firm_order:4,bound:5,renewal_pending:3,expired_review:0,closed_ntu:0};
  return map[s] || 0;
}
...
function nameSimilarity(a,b){
  ...
  var inter = at.filter(function(x){ return bt.includes(x); }).length;
  var union = new Set(at.concat(bt)).size || 1;
  return inter / union;
}

function scoreRiskMatch(draft, risk){
  var score = 0;
  var ns = nameSimilarity(draft.assured_name, risk.assured_name || risk.display_name);
  score += ns * 60;
  if(draft.product && risk.product && draft.product.toLowerCase() === String(risk.product).toLowerCase()) score += 15;
  if(draft.accounting_year && risk.accounting_year && String(draft.accounting_year) === String(risk.accounting_year)) score += 10;
  if(draft.inception_date && risk.inception_date && draft.inception_date.slice(0,7) === String(risk.inception_date).slice(0,7)) score += 10;
  if(draft.handler && risk.handler && draft.handler.toLowerCase() === String(risk.handler).toLowerCase()) score += 5;
  return Math.round(score);
}

function renderIngestRiskMatches(matches, draft){
  var wrap = document.getElementById('en-risk-matches');
  var msg = document.getElementById('en-risk-match-msg');
  if(!wrap) return;
  var linked = window._ingestLinkedRiskId;
  if(msg){
    if(linked) msg.textContent = 'Linked to existing risk #' + linked + '. Saving will update that risk and attach tasks there.';
    else if(matches && matches.length) msg.textContent = 'Choose an existing risk if this email looks like an update rather than a new placement.';
    else msg.textContent = 'No strong backend matches found. Save will create a new risk.';
  }
  if(!matches || !matches.length){
    wrap.innerHTML = '<div class="muted" style="font-size:11px">No strong matches yet. Generate a draft or edit the assured/product/year and click Find matches.</div>';
    return;
  }
  wrap.innerHTML = matches.map(function(m){
    var active = linked === m.id;
    var tone = m.score >= 80 ? 'var(--ok)' : m.score >= 60 ? 'var(--warn)' : 'var(--text2)';
    return '<div style="border:1px solid '+(active?'var(--acc)':'var(--border)')+';border-radius:8px;padding:10px;background:'+(active?'var(--acc-bg)':'var(--surface)')+'">'+
      '<div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">'+
        '<div><div style="font-size:12px;font-weight:600">'+escapeHtml(m.display_name || m.assured_name || ('Risk #' + m.id))+'</div>'+
        '<div class="muted" style="font-size:11px;margin-top:2px">#'+m.id+' · '+escapeHtml(m.product || 'No product')+' · '+escapeHtml(m.status_label || m.status || '')+' · '+escapeHtml(String(m.accounting_year || ''))+'</div>'+
        '<div class="muted" style="font-size:11px;margin-top:2px">Premium: '+fmtMoneySmart(m.gross_premium, m.currency || 'USD')+' · Handler: '+escapeHtml(m.handler || '—')+'</div></div>'+
        '<div style="text-align:right;flex-shrink:0"><div style="font-size:12px;font-weight:700;color:'+tone+'">'+m.score+'</div><div class="muted" style="font-size:10px">match score</div>'+
        '<button class="btn sm '+(active?'primary':'')+'" style="margin-top:6px" onclick="selectIngestRiskMatch('+m.id+')">'+(active?'Linked':'Link to this risk')+'</button></div>'+
      '</div></div>';
  }).join('');
}

function selectIngestRiskMatch(id){
  window._ingestLinkedRiskId = id;
  if(Array.isArray(window._ingestRiskMatches)) renderIngestRiskMatches(window._ingestRiskMatches, getCurrentIngestRiskPayload());
  var saveBtn = document.getElementById('en-risk-save-btn');
  if(saveBtn) saveBtn.textContent = 'Update linked risk + tasks →';
}

function clearIngestRiskLink(){
  window._ingestLinkedRiskId = null;
  if(Array.isArray(window._ingestRiskMatches)) renderIngestRiskMatches(window._ingestRiskMatches, getCurrentIngestRiskPayload());
  var saveBtn = document.getElementById('en-risk-save-btn');
  if(saveBtn) saveBtn.textContent = 'Save risk + tasks →';
}

async function findIngestRiskMatches(){
  var btn = document.getElementById('en-risk-match-btn');
  var draft = getCurrentIngestRiskPayload();
  var assured = (draft.assured_name || '').trim();
  if(!assured){
    renderIngestRiskMatches([], draft);
    var m0 = document.getElementById('en-risk-match-msg'); if(m0) m0.textContent = 'Enter or generate an assured name first.';
    return;
  }
  try {
    if(btn){ btn.disabled = true; btn.textContent = 'Finding...'; }
    var data = await apiFetch('/risks?q=' + encodeURIComponent(assured) + '&limit=25');
    var rows = (data.items || []).map(function(r){ r.score = scoreRiskMatch(draft, r); return r; })
      .filter(function(r){ return r.score >= 45; })
      .sort(function(a,b){ return b.score - a.score; })
      .slice(0,5);
    window._ingestRiskMatches = rows;
    if(rows.length && !window._ingestLinkedRiskId && rows[0].score >= 80) window._ingestLinkedRiskId = rows[0].id;
    renderIngestRiskMatches(rows, draft);
  } catch(e) {
    console.error(e);
    var m1 = document.getElementById('en-risk-match-msg'); if(m1) m1.textContent = 'Match lookup failed: ' + e.message;
  } finally {
    if(btn){ btn.disabled = false; btn.textContent = 'Find matches ↗'; }
  }
}

async function generateIngestRiskDraft(){
  var msg = document.getElementById('en-risk-msg');
  var btn = document.getElementById('en-risk-generate-btn');
  try {
    if(msg) msg.textContent = 'Generating risk draft...';
    if(btn){ btn.disabled = true; btn.textContent = 'Generating...'; }
    var selectedIns = '';
    var sel = document.getElementById('en-insured-select');
    if(sel && sel.value && sel.selectedOptions && sel.selectedOptions[0]) selectedIns = sel.selectedOptions[0].textContent || '';
    var user = JSON.stringify({
      matched_insured: selectedIns,
      proposed_new_insured: (document.getElementById('single-new-name')||{}).value || '',
      date: document.getElementById('en-date').value || '',
      handler: document.getElementById('en-handler').value || '',
      parties: document.getElementById('en-parties').value || '',
      summary: document.getElementById('en-summary').value || '',
      actions: document.getElementById('en-actions').value || '',
      status_change: document.getElementById('en-status').value || '',
      email_meta: window._lastEmailData || {},
      backend_note: window._lastNoteData || {}
    });
    var text = await aiText({
      model:'claude-sonnet-4-20250514',
      max_tokens:700,
      system:`You are extracting a Lloyd's broker risk draft from an email ingest review. Return JSON only with these keys: assured_name, product, status, accounting_year, inception_date, expiry_date, currency, gross_premium, brokerage_pct, retained_pct, estimated_gbp_commission, notes, needs_review, review_reason. Rules: do not invent numbers. Leave unknown values null or empty. status must be one of submission, in_market, quoted, firm_order, bound, renewal_pending. accounting_year should be numeric if inferable from inception/renewal timing, otherwise current year. dates must be YYYY-MM-DD when known. notes should be a concise operational note.`,
      user:user
    });
    var fields = {};
    try { fields = JSON.parse(text); } catch(e) {
      var m = text && text.match(/\{[\s\S]*\}/);
      if(m) fields = JSON.parse(m[0]);
    }
    fillIngestRiskDraft(fields || {});
    window._lastRiskDraft = fields || {};
    await findIngestRiskMatches();
...
function buildLinkedRiskPatch(existing, incoming){
  var patch = { notes: existing.notes || '' };
  var stamp = new Date().toISOString().slice(0,10);
  var append = '\n\n[Ingest update ' + stamp + ']\n' + (incoming.notes || 'Email ingest update');
  patch.notes = (patch.notes || '').trim() ? ((patch.notes || '').trim() + append) : append.trim();
  if(incoming.product && !existing.product) patch.product = incoming.product;
  if(incoming.handler && !existing.handler) patch.handler = incoming.handler;
  if(incoming.currency && !existing.currency) patch.currency = incoming.currency;
  if(incoming.brokerage_pct != null && (existing.brokerage_pct == null || existing.brokerage_pct === '')) patch.brokerage_pct = incoming.brokerage_pct;
  if(incoming.retained_pct != null && (existing.retained_pct == null || existing.retained_pct === '')) patch.retained_pct = incoming.retained_pct;
  if(incoming.estimated_gbp_commission != null && (existing.estimated_gbp_commission == null || existing.estimated_gbp_commission === '')) patch.estimated_gbp_commission = incoming.estimated_gbp_commission;
  if(incoming.inception_date && !existing.inception_date) patch.inception_date = incoming.inception_date;
  if(incoming.expiry_date && !existing.expiry_date) patch.expiry_date = incoming.expiry_date;
  if(incoming.gross_premium != null && (!existing.gross_premium || riskStatusRank(incoming.status) > riskStatusRank(existing.status))) patch.gross_premium = incoming.gross_premium;
  if(riskStatusRank(incoming.status) > riskStatusRank(existing.status)) patch.status = incoming.status;
  if(incoming.ai_extracted) patch.ai_extracted = incoming.ai_extracted;
  patch.needs_review = false;
  patch.review_reason = '';
  return patch;
}

async function saveIngestRiskDraft(){
  var payload = getCurrentIngestRiskPayload();
  if(!payload.assured_name){ showNotice('Assured name required','err'); return; }
  var saveBtn = document.getElementById('en-risk-save-btn');
  var msg = document.getElementById('en-risk-msg');
  var taskMsg = document.getElementById('en-task-msg');
  try {
    var linkedId = window._ingestLinkedRiskId;
    if(saveBtn){ saveBtn.disabled = true; saveBtn.textContent = linkedId ? 'Updating...' : 'Saving...'; }
    var row;
    if(linkedId){
      var existing = await apiFetch('/risks/' + linkedId);
      var patch = buildLinkedRiskPatch(existing, payload);
      row = await apiFetch('/risks/' + linkedId, {method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify(patch)});
      window._lastSavedRiskId = row.id;
      var linkedTaskResult = await saveIngestTaskDrafts(row.id);
      showNotice('✓ Existing risk updated: ' + (row.assured_name || payload.assured_name) + (linkedTaskResult.saved ? ' · ' + linkedTaskResult.saved + ' task' + (linkedTaskResult.saved===1?'':'s') + ' created' : ''), 'ok');
      if(msg) msg.textContent = 'Linked to backend risk #' + row.id + ' and updated with newer draft details.';
      if(taskMsg && linkedTaskResult.saved){ taskMsg.textContent = linkedTaskResult.saved + ' task draft' + (linkedTaskResult.saved===1?'':'s') + ' saved to WIP.'; }
    } else {
      row = await apiFetch('/risks', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)});
      window._lastSavedRiskId = row.id;
      var taskResult = await saveIngestTaskDrafts(row.id);
      showNotice('✓ Risk draft saved to backend: ' + (row.assured_name || payload.assured_name) + (taskResult.saved ? ' · ' + taskResult.saved + ' task' + (taskResult.saved===1?'':'s') + ' created' : ''), 'ok');
      if(msg) msg.textContent = 'Saved to Railway risk ledger' + (taskResult.saved ? ' with ' + taskResult.saved + ' task draft' + (taskResult.saved===1?'':'s') + '.' : '.');
      if(taskMsg && taskResult.saved){ taskMsg.textContent = taskResult.saved + ' task draft' + (taskResult.saved===1?'':'s') + ' saved to WIP.'; }
    }
    clearIngestRiskLink();
  } catch(e) {
    showNotice('Risk save failed: ' + e.message, 'err');
    if(msg) msg.textContent = 'Save failed: ' + e.message;
  } finally {
    if(saveBtn){ saveBtn.disabled = false; saveBtn.textContent = 'Save risk + tasks →'; }
  }
}
...
function saveEmailNoteNewInsured() {
  showNotice('Create insured from email — use the entity panel', 'warn');
}
...
function toggleSingleCreateInsured() {
  var el = document.getElementById('single-create-insured');
  if (el) el.style.display = el.style.display === 'none' ? 'block' : 'none';
}
